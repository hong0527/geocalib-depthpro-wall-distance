"""
draft_v3.docx — KCC 2026 폰트/크기 규정 완전 준수 + Figure 1 (파이프라인) 추가

실측 기반 KCC 원본 규정:
  - 한글 폰트: 함초롬돋움 (494회 사용 / 원본 표준)
  - 본문:    10.5 pt  (25회 / 표준)
  - 섹션 헤딩: 14 pt
  - 소섹션:    11 pt
  - 제목:     20 pt
...
구조:
  [Section 1 — 1단] 제목 / 요약 / Keywords / Abstract
  [Section 2 — 2단] 본문 1~5 + 참고문헌 + Figure 1/2/3/4
"""
import os, shutil
...
DST  = f'{BASE}/draft_v3.docx'

FIGS_DIR = '/Users/honghwasu/Desktop/research_project/output/paper_figures'
FIG1 = f'{FIGS_DIR}/fig1_pipeline.png'
FIG2 = f'{FIGS_DIR}/fig2_3d_visualization_v5.png'
FIG3 = f'{FIGS_DIR}/fig3_focal_histogram_v2.png'
FIG4 = f'{FIGS_DIR}/fig4_manual_vs_auto_v2.png'

# KCC 규정 폰트 크기 (half-point 단위로 변환)
# 샘플 PDF(sample1.pdf) 실측 매칭 — 본문 10pt 기준
SIZE_TITLE     = 16    # 주제목
SIZE_TITLE_EN  = 14    # 영문 제목
SIZE_SECTION   = 12    # 섹션 헤딩
SIZE_SUBSEC    = 10    # 소섹션 (본문과 동일, bold로 구분)
SIZE_BODY      = 10    # 본문
SIZE_SUMMARY   = 10    # 요약
SIZE_TABLE     = 9     # 표 내부
SIZE_REF       = 9     # 참고문헌
...
# Section 구조 분리 (원본 그대로 사용)
section_break_paras, other, final_sectPr = [], [], None
for child in list(body):
    if child.tag == qn('w:sectPr'): final_sectPr = child
    elif child.tag == qn('w:p'):
        pPr = child.find(qn('w:pPr'))
        if pPr is not None and pPr.find(qn('w:sectPr')) is not None:
            section_break_paras.append(child)
        else: other.append(child)
    elif child.tag == qn('w:tbl'): other.append(child)

for c in other: body.remove(c)
for sp in section_break_paras:
    for r in list(sp):
        if r.tag != qn('w:pPr'): sp.remove(r)
...
def make_para(text, size, bold=False, italic=False,
    ...
    if align is not None:
        jc = OxmlElement('w:jc')
        jc.set(qn('w:val'), {
            WD_ALIGN_PARAGRAPH.CENTER:'center', WD_ALIGN_PARAGRAPH.LEFT:'left',
            WD_ALIGN_PARAGRAPH.JUSTIFY:'both', WD_ALIGN_PARAGRAPH.RIGHT:'right'
        }[align])
        pPr.append(jc)
    if indent_mm:
    ...
    rFonts.set(qn('w:eastAsia'), FONT_KOR)
    rFonts.set(qn('w:hAnsi'), FONT_ENG)
    rFonts.set(qn('w:cs'), FONT_ENG)
    rPr.append(rFonts)
    r.append(rPr)
    t = OxmlElement('w:t')
...
def table_elem(headers, rows, col_widths_mm):
    ...
    tr = OxmlElement('w:tr')
    for h in headers: tr.append(cell(h, bold=True))
    tbl.append(tr)
    for row in rows:
        tr = OxmlElement('w:tr')
        for v in row: tr.append(cell(v))
        tbl.append(tr)
    return tbl

def insert_before(anchor, *els):
    for el in els: anchor.addprevious(el)
...
# KCC 규정: ① 국문제목 ② 영문제목 ③ 요약 만 1단. Keywords/Abstract는 규정에 없으므로 제외.
insert_before(sec_break, *s1)

# ============================================================
# SECTION 2 (2단): 본문 + 참고문헌 + 그림
# ============================================================
s2 = []
...
# Figure 2 caption (image will be auto-inserted before this paragraph)
...
# 그림 3 제거 (분량 3페이지 준수를 위해 본문 서술로 대체)
...
# ============================================================
# 그림 삽입 (caption 앞에 이미지 배치)
# ============================================================
doc = Document(DST)
fig_map = [
    # Figure 1 제거 — 추후 사용자가 직접 제작 예정 (빈 공간 유지)
    ('그림 2.', FIG2, 55),
]
# Figure 4 (수동 vs 자동 막대) 제거 — Table 3만으로 충분히 전달. 3페이지 준수 목적.
