...
class WallClickSelector:
    ...
    def compute(self):
        ...
        gt_adj = 3240
        err = abs(dist_mm - gt_adj) / gt_adj * 100
        self.result = {'dist_mm': dist_mm, 'error_pct': round(err,1), 'parallelism': round(abs(dot), 3)}
        txt = f"수동: {dist_mm}mm | GT: {gt_adj}mm | 오차: {err:.1f}% | 자동: 4090mm"
...
if __name__ == '__main__':
    depth = np.load('output/new_depth_maps/고성2_depth.npy')
    img = plt.imread('/Users/honghwasu/Downloads/고성방사진2.jpeg')
    f = 1002.5

    print("고성2 수동 벽 측정 | GT(마감면)=3240mm | 자동=4090mm")
    print("벽 한가운데를 클릭! 유리문 말고 벽!")

    sel = WallClickSelector(img, depth, f, "고성2 | GT=3240mm | 자동=4090mm")
    res = sel.show()
    if res:
        print(f"\n수동: {res['dist_mm']}mm | 오차: {res['error_pct']}% | 평행: {res['parallelism']}")
        print(f"자동: 4090mm → 수동: {res['dist_mm']}mm")
