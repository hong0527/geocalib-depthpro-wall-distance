"""
장애물 있는 사진에서 수동 벽 클릭 측정
RANSAC 결과와 비교하여 수동이 더 나은지 확인
"""
import os
...
# 장애물 있는 사진 5장 (RANSAC 오차 큰 순)
test_images = [
    ('012', 3300, '침실(발코니) - RANSAC 45.0% - 발코니문 정면'),
    ('048', 3000, '침실(작은) - RANSAC 35.1% - 붙박이장 정면'),
    ('004', 3600, '안방 - RANSAC 29.6% - 발코니유리 주의'),
    ('009', 3600, '안방 - RANSAC 19.8% - 발코니유리 앞뒤방향'),
    ('038', 3600, '거실 - RANSAC 16.2% - 주방 빌트인 아일랜드'),
]
...
for img_id, gt_mm, desc in test_images:
    ...
    print(f"  벽 부분만 클릭하세요 (가구/유리문 피하기)")
    ...
    ax.imshow(img)
    ax.set_title(f"[{img_id}] {desc}\n왼쪽 벽 클릭 (가구/유리문 피하기)", fontsize=12)
    fig.canvas.mpl_connect('button_press_event', make_onclick(clicks, result_done))
    plt.show()

# 최종 비교
print(f"\n{'='*60}")
print(f"RANSAC vs 수동 클릭 비교")
print(f"{'='*60}")
ransac_errs = {'012':45.0, '048':35.1, '004':29.6, '009':19.8, '038':16.2}
for r in results:
    re = ransac_errs.get(r['id'], 0)
    better = "수동 승" if r['pct'] < re else "RANSAC 승"
    print(f"  [{r['id']}] RANSAC:{re}% → 수동:{r['pct']}% ({better}) - {r['desc']}")
